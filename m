...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id DB3002730E2
	for <lists+linux-rtc@lfdr.de>; Mon, 21 Sep 2020 19:30:06 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728040AbgIUR3r (ORCPT <rfc822;lists+linux-rtc@lfdr.de>);
        Mon, 21 Sep 2020 13:29:47 -0400
Received: from mslow2.mail.gandi.net ([217.70.178.242]:37396 "EHLO
        mslow2.mail.gandi.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1727525AbgIUR3m (ORCPT
        <rfc822;linux-rtc@vger.kernel.org>); Mon, 21 Sep 2020 13:29:42 -0400
Received: from relay1-d.mail.gandi.net (unknown [217.70.183.193])
        by mslow2.mail.gandi.net (Postfix) with ESMTP id DAEF13A6A9C;
        Mon, 21 Sep 2020 17:18:03 +0000 (UTC)
X-Originating-IP: 90.65.88.165
Received: from localhost (lfbn-lyo-1-1908-165.w90-65.abo.wanadoo.fr [90.65.88.165])
        (Authenticated sender: alexandre.belloni@bootlin.com)
        by relay1-d.mail.gandi.net (Postfix) with ESMTPSA id 30015240012;
        Mon, 21 Sep 2020 17:17:42 +0000 (UTC)
From:   Alexandre Belloni <alexandre.belloni@bootlin.com>
To:     Alessandro Zummo <a.zummo@towertech.it>,
        Alexandre Belloni <alexandre.belloni@bootlin.com>
Cc:     linux-kernel@vger.kernel.org, linux-rtc@vger.kernel.org
Subject: [PATCH 3/3] rtc: rv3032: Add a driver for Microcrystal RV-3032
Date:   Mon, 21 Sep 2020 19:17:34 +0200
Message-Id: <20200921171735.1191342-3-alexandre.belloni@bootlin.com>
...
index 000000000000..1b0b0b5bf83a
--- /dev/null
...
+
+	return regmap_update_bits(rv3032->regmap, RV3032_OFFSET, RV3032_OFFSET_MSK,
...
+	u8 *buf = val;
+
+	ret = regmap_read(priv, RV3032_CTRL1, &ctrl1);
+	if (ret)
+		return ret;
+
+	if (!(ctrl1 & RV3032_CTRL1_EERD)) {
+		ret = regmap_update_bits(priv, RV3032_CTRL1,
+					 RV3032_CTRL1_EERD, RV3032_CTRL1_EERD);
+		if (ret)
+			return ret;
+
+		ret = regmap_read_poll_timeout(priv, RV3032_TLSB, status,
...
+
+	for (i = 0; i < bytes; i++) {
+		ret = regmap_write(priv, RV3032_EEPROM_ADDR, RV3032_EEPROM_USER + offset + i);
+		if (ret)
+			goto restore_eerd;
+
+		ret = regmap_write(priv, RV3032_EEPROM_DATA, buf[i]);
+		if (ret)
+			goto restore_eerd;
+
+		ret = regmap_write(priv, RV3032_EEPROM_CMD, 0x0);
+		if (ret)
+			goto restore_eerd;
+
+		ret = regmap_write(priv, RV3032_EEPROM_CMD,
...
+restore_eerd:
+	if (!(ctrl1 & RV3032_CTRL1_EERD)) {
+		err = regmap_update_bits(priv, RV3032_CTRL1, RV3032_CTRL1_EERD, 0);
+		if (err && !ret)
+			ret = err;
+	}
+
...
+			goto restore_eerd;
+
+		ret = regmap_read(priv, RV3032_EEPROM_DATA, &data);
+		if (ret)
+			goto restore_eerd;
+		buf[i] = data;
+	}
...
+	if (!device_property_read_u32(dev, "trickle-voltage-mV", &voltage)) {
...
+			ret = regmap_update_bits(rv3032->regmap, RV3032_PMU,
+						 RV3032_PMU_TCR | RV3032_PMU_TCM | RV3032_PMU_BSM,
+						 FIELD_PREP(RV3032_PMU_TCR, i));
+			if (ret)
+				return ret;
+		} else {
